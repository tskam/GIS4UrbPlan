# Introduction

QGIS (formally known as Quantum GIS) is a full-features open source GIS software. It is the GIS tool used in this course.

This section provides you with step-by-step guide on how to download and install QGIS in your window-based computer (i.e. desktop or laptop).

## Downloading QGIS installer

-   Launch the web browser.

-   In the address bar at the top of the window, enter http://qgis.org/ and press **Enter**.

The website should look something like this:

![](img/image1.jpg)

-   Click on **Download** button.

Your screen should look similar to the figure below.

![](img/image1-1.jpg)

-   Scroll downward and click on ![](img/image1-2.jpg){width="140" height="26"}

Your screen should look similar to the figure below.

![](img/image2.jpg)

-   Click on **Latest Version for Windows (3.40)**.

Note:

-   The version appears on your screen might be slightly different from the screenshot. It is alright at long as it is the latest version.

-   Change to other OS if your computer is not running Windows OS.

After a few seconds (depend on your network speed), QGIS installer will start downloading.

![](img/image3.jpg){width="350"}

Be patient, the downloading will take a few minutes depend on the speed of your internet.

## Installing QGIS

In this section, you will learn how to install QGIS into your computer. Note that the instructions are based on Windows machine, if your computer is not Windows based, please adjust according to the OS of your computer.

-   Find the QGIS installer on your computer, right-click and select **Open with** -\> **Windows installer** from the context menu to launch the setup.

After a few second, the Setup Wizard dialog window appears.

![](img/image5.jpg)

-   Click on the **Next** button.

The End-User License Agreement dialog window appears.

![](img/image6.jpg)

-   Click on the checkbox in front of **I accept the terms in the License Agreement** button.

-   Then, click on **Next** button.

The Destination Folder dialog window appears.

![](img/image7.jpg)

-   Ensure that the Destination Folder is at `C:\Program Files\QGIS 3.40.2\`

-   Keep both checkboxes selected.

-   Click on the **Next** button.

The **Ready to install QGIS 3.40.2 'Bratislava'** dialog window appears.

![](img/image8.jpg)

-   Click on **Install** button to start the installation process.

![](img/image9.jpg)

-   Provide the administrator right when prompted.

QGIS will begin to install. It may take a few minutes to install so be patient.

When the installation is completed, a Setup Wizard window look similar to the screenshot below appears.

![](img/image10.jpg)

-   Click on **Finish** button.

Congratulations! You have installed QGIS successfully!
